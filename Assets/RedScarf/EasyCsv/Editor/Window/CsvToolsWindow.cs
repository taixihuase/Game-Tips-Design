...
namespace RedScarf.EasyCsvEditor
{
    ...
    public class CsvToolsWindow : EditorWindow
    {
            ...
            using (new GUILayout.AreaScope(rect))
            {
                ...
                using (new GUILayout.HorizontalScope())
                {
                    ...
                    if (GUILayout.Button("Csv -> Class"))
                    {
                        ...
                        {
                            var path = outputFolderPath + "/" + item.Key.ToUpper() + ".cs";
        ...
        string GetClassStr(CsvTable table,string csvName)
        {
            ...
            for (var i=0;i<len;i++)
            {
                var fieldDesc = table.Read(0,i);
                var fieldName = table.Read(1, i);
                var fieldType = table.Read(2,i);
                var fieldNode = new FieldNode(fieldDesc, fieldName, fieldType);
