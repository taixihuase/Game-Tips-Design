...
namespace RedScarf.EasyCSV
{
    ...
    public static class CsvHelper
    {
        ...
        public static Dictionary<int, T> PaddingData<T>(string csvName) where T : new()
        {
            Dictionary<int, T> resultDic = new Dictionary<int, T>();
            var table = Get(csvName);
            ...
            for (int i = 3; i < rowCount; i++)
            {
                ...
                foreach (var field in fields)
                {
                    var valueStr = table.Read(i, field.Name);
                    if (string.IsNullOrEmpty(valueStr)) continue;
                    ...
                    try
                    {
                        ...
                        if (field.Name == "id")
                        {
                            resultDic.Add((int)value, rowData);
                        }
                    }
                    catch (Exception e)
                    {
